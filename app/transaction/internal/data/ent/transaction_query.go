...
type TransactionQuery struct {
	...
	withTxItems *TxItemQuery
	withTxOrder *OrderQuery
	withFKs     bool
...
func (tq *TransactionQuery) QueryTxItems() *TxItemQuery {
	query := (&TxItemClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withTxItems: tq.withTxItems.Clone(),
		withTxOrder: tq.withTxOrder.Clone(),
...
// WithTxItems tells the query-builder to eager-load the nodes that are connected to
// the "txItems" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithTxItems(opts ...func(*TxItemQuery)) *TransactionQuery {
	query := (&TxItemClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTxItems = query
	return tq
}

// WithTxOrder tells the query-builder to eager-load the nodes that are connected to
// the "txOrder" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithTxOrder(opts ...func(*OrderQuery)) *TransactionQuery {
	query := (&OrderClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTxOrder = query
	return tq
}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withTxItems; query != nil {
		if err := tq.loadTxItems(ctx, query, nodes,
			func(n *Transaction) { n.Edges.TxItems = []*TxItem{} },
			func(n *Transaction, e *TxItem) { n.Edges.TxItems = append(n.Edges.TxItems, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withTxOrder; query != nil {
		if err := tq.loadTxOrder(ctx, query, nodes,
			func(n *Transaction) { n.Edges.TxOrder = []*Order{} },
			func(n *Transaction, e *Order) { n.Edges.TxOrder = append(n.Edges.TxOrder, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadTxItems(ctx context.Context, query *TxItemQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *TxItem)) error {
	...
	query.withFKs = true
	query.Where(predicate.TxItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(transaction.TxItemsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.transaction_tx_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "transaction_tx_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "transaction_tx_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TransactionQuery) loadTxOrder(ctx context.Context, query *OrderQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Order)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Transaction)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Order(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(transaction.TxOrderColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.transaction_tx_order
		if fk == nil {
			return fmt.Errorf(`foreign-key "transaction_tx_order" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "transaction_tx_order" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tq *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(transaction.Table, transaction.Columns, sqlgraph.NewFieldSpec(transaction.FieldID, field.TypeInt64))
	_spec.From = tq.sql
