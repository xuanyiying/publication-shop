...
// The TxItemFunc type is an adapter to allow the use of ordinary
// function as TxItem mutator.
type TxItemFunc func(context.Context, *ent.TxItemMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TxItemFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TxItemMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TxItemMutation", m)
}
