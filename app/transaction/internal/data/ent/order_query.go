...
func (oq *OrderQuery) QueryTransaction() *TransactionQuery {
	query := (&TransactionClient{config: oq.config}).Query()
...
func (oq *OrderQuery) QueryTxItems() *TxItemQuery {
	query := (&TxItemClient{config: oq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := oq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (oq *OrderQuery) Clone() *OrderQuery {
	...
	return &OrderQuery{
		...
		withTransaction: oq.withTransaction.Clone(),
		withTxItems:     oq.withTxItems.Clone(),
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := oq.withTransaction; query != nil {
		if err := oq.loadTransaction(ctx, query, nodes,
			func(n *Order) { n.Edges.Transaction = []*Transaction{} },
			func(n *Order, e *Transaction) { n.Edges.Transaction = append(n.Edges.Transaction, e) }); err != nil {
			return nil, err
		}
	}
	if query := oq.withTxItems; query != nil {
		if err := oq.loadTxItems(ctx, query, nodes,
			func(n *Order) { n.Edges.TxItems = []*TxItem{} },
			func(n *Order, e *TxItem) { n.Edges.TxItems = append(n.Edges.TxItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (oq *OrderQuery) loadTransaction(ctx context.Context, query *TransactionQuery, nodes []*Order, init func(*Order), assign func(*Order, *Transaction)) error {
	...
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.TransactionColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.order_transaction
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_transaction" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_transaction" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (oq *OrderQuery) loadTxItems(ctx context.Context, query *TxItemQuery, nodes []*Order, init func(*Order), assign func(*Order, *TxItem)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.TxItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(order.TxItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.order_tx_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "order_tx_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "order_tx_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
