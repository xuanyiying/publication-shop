...
import (
	...
	"github.com/xuanyiying/publication-shop/app/book/service/internal/data/ent/migrate"
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *BookStockClient) DeleteOneID(id int) *BookStockDeleteOne {
...
func (c *BookStockClient) Get(ctx context.Context, id int) (*BookStock, error) {
	return c.Query().Where(bookstock.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *BookStockClient) GetX(ctx context.Context, id int) *BookStock {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
