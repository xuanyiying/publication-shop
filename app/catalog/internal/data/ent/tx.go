...
func (tx *Tx) init() {
	...
	tx.Classic = NewClassicClient(tx.config)
	tx.Language = NewLanguageClient(tx.config)
	tx.PublicationAuthor = NewPublicationAuthorClient(tx.config)
	tx.PublicationDetail = NewPublicationDetailClient(tx.config)
	tx.PublicationImg = NewPublicationImgClient(tx.config)
	tx.PublicationInfo = NewPublicationInfoClient(tx.config)
	tx.PublicationOrg = NewPublicationOrgClient(tx.config)
	tx.PublicationStock = NewPublicationStockClient(tx.config)
	tx.PublicationXCatalog = NewPublicationXCatalogClient(tx.config)
	tx.PublicationXClassic = NewPublicationXClassicClient(tx.config)
	tx.PublicationXLanguage = NewPublicationXLanguageClient(tx.config)
}
